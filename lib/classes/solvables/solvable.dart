...
abstract class Solvable {
  ...
  int get fakeSolutionsLength => maxFakeSolutions == -1
      ? app.cookies.numberOfPossibleSolutions.value - 1
      : min(app.cookies.numberOfPossibleSolutions.value - 1, maxFakeSolutions);

  // CONSTRUCTOR ===============================================================

  /// Returns the instance of [Solvable] generated using the seed.
  Solvable({int? seed, String? localizationPrefix}) {
    this.localizationPrefix = localizationPrefix.withUnderscore ?? "";
  }
  ...
  void prepare() {
    ...
    int remainingFakeSolutions =
        fakeSolutionsLength - possibleSolutions.length + 1;
