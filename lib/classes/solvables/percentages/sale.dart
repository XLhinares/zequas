...
class Sale extends Solvable with UsualFakes {
  ...
  Sale({super.localizationPrefix}) {
    ...
    usualFakeSolutions.add(
        (value * (1 + [1, -1][type] * percentage / 100)).toStringAsFixed(2));
  ...
  String defaultFakeSolution() {
    return (result + 0.2 * value * (0.5 - random.nextDouble()))
        .toStringAsFixed(2);
  }

  @override
  String generateQuestion() {
    return "$localizationPrefix${["sales_down", "sales_up"][type]}".trParams(
        {"value": value.toString(), "percentage": percentage.toString()});
  }
