...
class GameCategory extends Translatable {
  ...
  Map<String, Map<String, String>> get subTranslations {
    final Map<String, Map<String, String>> mergedTranslations = rawTranslations;

    dlog(name);
    dlog(mergedTranslations.toString());

    // Adds the translations of the modes.

    for (final mode in modes) {
      for (final locale in mergedTranslations.keys) {
        mergedTranslations[locale]?.addAll(mode.translations[locale] ?? {});
      }
    }
    dlog(mergedTranslations.toString());
